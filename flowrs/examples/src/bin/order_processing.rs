...
use chrono::Utc;
use flowrs_core::{
    ActionType, FlowrsError, Node, NodeId, NodeOutcome, RetryNode, Workflow, node,
};
use rand::Rng;
...
impl Node<OrderContext, OrderAction> for ValidateOrderNode {
    ...
    async fn process(&self, ctx: &mut OrderContext) -> Result<NodeOutcome<Self::Output, OrderAction>, FlowrsError> {
...
impl Node<OrderContext, OrderAction> for ProcessPaymentNode {
    ...
    async fn process(&self, ctx: &mut OrderContext) -> Result<NodeOutcome<Self::Output, OrderAction>, FlowrsError> {
        ...
        info!("Processing payment for order {} (attempt {})", ctx.order_id, ctx.payment_attempts);
        ...
        if self.should_fail {
            let note = format!("Payment processing failed on attempt {}", ctx.payment_attempts);
            ctx.add_note(&note);
            warn!("{}", note);
            return Err(FlowrsError::node_execution(
                self.id(),
        ...
        ctx.add_note(&format!("Payment processed successfully on attempt {}", ctx.payment_attempts));

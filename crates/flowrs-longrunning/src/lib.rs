//! Support for long-running processes with checkpoints in the Flowrs framework.
//!
...
where
    ...
    F: Fn(Option<State>, &mut Context) -> Result<LongRunningOutcome<Output, State>, FlowrsError>
        + Send
        + Sync
        + 'static,
{
    ...
    S: StateStore,
{
    ...
    pub async fn execute_node<LRN>(
        ...
        node: &LRN,
        ctx: &mut Context,
    ) -> Result<NodeOutcome<Action, Action>, FlowrsError>
    where
        LRN: LongRunningNode<Context, Action>,
    {
        let node_id = node.id();
    ...
    pub async fn has_suspended_state(&self, node_id: NodeId) -> Result<bool, FlowrsError> {
        self.state_store.has_state(node_id).await
    }

    /// Get all node IDs with suspended state
    pub async fn get_suspended_nodes(&self) -> Result<Vec<NodeId>, FlowrsError> {
...
pub trait StateStore: Send + Sync + 'static {
    ...
    async fn get_state<T: for<'de> Deserialize<'de> + Send + Sync + 'static>(
        &self,
        node_id: NodeId,
    ) -> Result<Option<T>, FlowrsError>;

    /// Check if a node has saved state
    async fn has_state(&self, node_id: NodeId) -> Result<bool, FlowrsError>;

    /// Remove a node's state
    async fn remove_state(&self, node_id: NodeId) -> Result<(), FlowrsError>;

    /// Get all node IDs with saved states
    async fn get_all_node_ids(&self) -> Result<Vec<NodeId>, FlowrsError>;
}
...
impl StateStore for InMemoryStateStore {
    async fn save_state<T: Serialize + Send + Sync + 'static>(
        ...
        let mut states = self
            .states
            .lock()
            .map_err(|e| FlowrsError::Other(format!("Failed to acquire mutex lock: {}", e)))?;

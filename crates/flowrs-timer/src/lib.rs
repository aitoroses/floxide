//! # Flowrs Timer
//!
//! Timer-based node extensions for the Flowrs framework.
//!
...
impl Schedule {
    ...
    pub fn next_execution(&self) -> Result<DateTime<Utc>, FlowrsError> {
        let now = Utc::now();
        
        match self {
            Schedule::Once(time) => {
                if time <= &now {
                    Err(FlowrsError::Other("Scheduled time has already passed".to_string()))
                } else {
                    Ok(*time)
                }
            },
            ...
            Schedule::Interval(duration) => {
                Ok(now + ChronoDuration::from_std(*duration).map_err(|e| 
                    FlowrsError::Other(format!("Failed to convert duration: {}", e))
                )?)
            },
            ...
            Schedule::Daily(hour, minute) => {
                ...
                next = next.with_hour(*hour).and_then(|dt| dt.with_minute(*minute))
                    .and_then(|dt| dt.with_second(0))
                    .and_then(|dt| dt.with_nanosecond(0))
                    .ok_or_else(|| FlowrsError::Other("Invalid hour or minute".to_string()))?;
            ...
            Schedule::Weekly(weekday, hour, minute) => {
                ...
                let days_until_weekday = (*weekday as i32 - now.weekday().num_days_from_monday() as i32 + 7) % 7;
            ...
            Schedule::Monthly(day, hour, minute) => {
                ...
                if *day <= 31 {
                    ...
                    match next.with_day(*day) {
                        Some(date) => next = date,
                        None => return Err(FlowrsError::Other(
                            format!("Invalid day {} for the current month", day)
                        )),
                    }
                    ...
                    if next <= now || *day < current_day {
                        ...
                        next = next.with_day(*day).ok_or_else(|| 
                            FlowrsError::Other(format!("Invalid day {} for next month", day))
                        )?;
                    }
                } else {
                    return Err(FlowrsError::Other(format!("Invalid day of month: {}", day)));
                }
            ...
            Schedule::Cron(_expression) => {
                ...
                Err(FlowrsError::Other("Cron expressions are not yet implemented".to_string()))
            },
        }
    }
    
    /// Calculate the duration until the next execution
    pub fn duration_until_next(&self) -> Result<Duration, FlowrsError> {
        let next = self.next_execution()?;
        let now = Utc::now();
        
        let duration = next.signed_duration_since(now);
        if duration.num_milliseconds() <= 0 {
            return Err(FlowrsError::Other("Scheduled time is in the past".to_string()));
        }
...
where
    Context: Send + Sync + 'static,
    Action: ActionType + Send + Sync + 'static + Default + Debug,
    F: Fn(&mut Context) -> Result<Action, FlowrsError> + Send + Sync + 'static,
{
    fn schedule(&self) -> Schedule {
        self.schedule.clone()
    }
    ...
    pub async fn execute(&self, ctx: &mut Context) -> Result<(), FlowrsError> {
        ...
        loop {
            let node = self.nodes.get(&current_node_id).ok_or_else(|| {
                FlowrsError::Other(format!("Node not found: {}", current_node_id))
            })?;
            ...
            let wait_duration = match node.schedule().duration_until_next() {
                ...
                Err(e) => {
                    warn!("Failed to calculate next execution time for node {}: {}", 
                        current_node_id, e);
            ...
            info!("Waiting {:?} until next execution of node {}", wait_duration, current_node_id);
            ...
            if let Some(next_node_id) = self.routes.get(&(current_node_id.clone(), action.clone())) {
                info!("Moving from node {} to node {}", current_node_id, next_node_id);
                current_node_id = next_node_id.clone();
            } else {
                // If there's no route for this action, use the default action
                if let Some(next_node_id) = self.routes.get(&(current_node_id.clone(), Action::default())) {
                    info!("No route found for action {:?}, using default route to node {}", 
                        action, next_node_id);
                    current_node_id = next_node_id.clone();
                } else {
                    // If there's no default route either, stop the workflow
                    warn!("No route found for node {} with action {:?} and no default route", 
    ...
    Action: ActionType + Send + Sync + 'static + Default + Debug,
{
    ...
    async fn process(&self, ctx: &mut Context) -> Result<NodeOutcome<Self::Output, Action>, FlowrsError> {
        if self.execute_immediately {
            ...
            info!("Waiting {:?} before executing node {}", wait_duration, self.id);
            sleep(wait_duration).await;

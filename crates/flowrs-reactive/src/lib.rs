//! Support for reactive patterns in the Flowrs framework.
//!
    ...
    R: ReactiveNode<Change, Context, Action> + Send + 'static,
{
    ...
    pub async fn start_watching(
        &self,
        mut ctx: Context,
    ) -> Result<impl Stream<Item = Action> + Unpin, FlowrsError> {
    ...
    Action: ActionType + Send + Sync + 'static + Debug,
{
    ...
    where
        ...
        Fut: Future<Output = Result<Action, FlowrsError>> + Send + 'static,
    {
    ...
    Action: ActionType + Send + Sync + 'static + Debug,
{
    async fn watch(
        &self,
    ) -> Result<Box<dyn Stream<Item = FileChange> + Send + Unpin>, FlowrsError> {
        let file_path = self.file_path.clone();
        let poll_interval = self.poll_interval;

        // Check if the file exists
        if !Path::new(&file_path).exists() {
            return Err(FlowrsError::Other(format!("File not found: {}", file_path)));
        }
...
where
    ...
    Context: Send + Sync + 'static,
    Action: ActionType + Send + Sync + 'static + Debug,
    WatchFn: Fn() -> Result<Box<dyn Stream<Item = Change> + Send + Unpin>, FlowrsError>
        + Send
        + Sync
        + 'static,
    ReactFn: Fn(Change, &mut Context) -> Result<Action, FlowrsError> + Send + Sync + 'static,
{
    id: NodeId,
